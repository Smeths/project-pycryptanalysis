...
# Substitute most frequent letters for most frequent bigrams
    ...
    for XX in sorted(ADFGVX_dict, key=ADFGVX_dict.get, reverse=True):
        ADFGVX_dict[XX] = ENG_freq_order[i]
        i = i + 1

# Create dictionary of ADFGVX bigrams matrix and frequencies
    pt_decipher_2 = []
    for n in xrange(0,len(pt_encipher)/2):
        pt_decipher_2.append(ADFGVX_dict[pt_decipher[2*n:2*n + 2]])

    pt_decipher_2_str = ''.join(list(pt_decipher_2))

# Check to see if current iteration value closer to 0.066
    
    chi_current = chi.run(pt_decipher_2_str)

    if chi_current < chi_max:
        chi_max = chi_current
        pt_deciphered = pt_decipher_2_str

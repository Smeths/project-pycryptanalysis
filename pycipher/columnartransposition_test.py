...
class ColTrans_test(Cipher):
    ...
    def encipher(self,string):
        ...
        if self.cipher=='': 
            ...
            self.cipher = string.upper()
   
        ret = ''
        ind = self.sortind(self.keyword)
        for i in range(len(self.keyword)):
            ret += self.cipher[ind.index(i)::len(self.keyword)]
        return ret
    ...
    def decipher(self,string):
        ...
        ret = ['_']*len(self.cipher)
        L,M = len(self.cipher),len(self.keyword)
        ind = self.unsortind(self.keyword)
        upto = 0
        for i in range(len(self.keyword)):
            thiscollen = L/M
            if ind[i]< L%M: thiscollen += 1
            ret[ind[i]::M] = self.cipher[upto:upto+thiscollen]
            upto += thiscollen
        return ''.join(ret)    
